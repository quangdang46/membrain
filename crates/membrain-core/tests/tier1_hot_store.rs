...
fn exact_lookup_hits_without_fetching_large_payloads() {
    ...
    store.seed(seed_record(1, 10, "recent note", 32_768));
...
fn recent_window_stays_session_local_and_bounded() {
    ...
    store.seed(seed_record(1, 10, "session-a old", 4_096));
    store.seed(seed_record(2, 20, "session-b", 4_096));
    store.seed(seed_record(3, 10, "session-a new", 4_096));
    store.seed(seed_record(4, 10, "session-a newest", 4_096));
...
fn default_recent_window_scans_past_interleaved_foreign_session_entries() {
    let mut store = Tier1HotMetadataStore::new(4);
    store.seed(seed_record(1, 10, "target older", 4_096));
    store.seed(seed_record(2, 20, "foreign newer", 4_096));
    store.seed(seed_record(3, 10, "target newest", 4_096));
    store.seed(seed_record(4, 30, "foreign newest-most", 4_096));
...
fn bounded_capacity_evicts_the_oldest_hot_metadata_entry() {
    ...
    store.seed(seed_record(1, 10, "oldest", 8_192));
    store.seed(seed_record(2, 10, "middle", 8_192));
    store.seed(seed_record(3, 10, "newest", 8_192));

    let oldest = store.exact_lookup(MemoryId(1));
    let newest = store.exact_lookup(MemoryId(3));
