...
- Duplicate-family routing is an encode-time identity decision, not an interference side effect. The observable outcome must distinguish whether the write created a new identity, joined or updated an existing duplicate family, or preserved a new identity via pattern separation despite high similarity.
...
- Fast-path observability for this contract must also expose bounded shortlist evidence such as candidates inspected plus whether any follow-on interference work was applied, skipped, or deferred.
- Convenience booleans like `was_duplicate` may exist on some interfaces, but they do not satisfy the contract unless a richer machine-readable write-path summary or inspect/explain surface preserves the route explanation.
