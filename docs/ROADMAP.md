...
- **Autobiographical continuity and self-relevance follow-ons** stay later-stage and bounded. They may refine retrieval and inspect behavior using existing goal, preference, timeline, belief, and task/session surfaces so the system can better explain why certain memories matter to the current long-horizon self, but they must remain derived, inspectable, conflict-aware, and explicitly non-authoritative compared with durable memory truth.
- **Source-trust and social-memory follow-ons** stay later-stage and explainability-first. They may add topic-aware source calibration, relationship context, or social-memory inspect surfaces after provenance, contradiction, and governance contracts are already stable, but they must not collapse disagreement into one opaque trust score, hide losing evidence, or become a policy shortcut.
- **Reconsolidation outcome and interference follow-ons** stay later-stage and audit-friendly. They may enrich how the system explains recall reopen outcomes, reinterpretation, and memory competition after the underlying reconsolidation, contradiction, and retrieval contracts are already proven, but they must not permit silent overwrite, hidden controller-state authority, or unbounded maintenance behavior.
- **Human-like evaluation and promotion framing** stays non-blocking and evidence-gated. Later-stage memory features should be judged by persistence quality, active-work continuity, procedural reuse, autobiographical continuity, interference resilience, source-trust calibration, schema usefulness, and dream-synthesis utility, and they should not be promoted to canonical behavior without measurable utility, bounded cost, and inspectable proof.
